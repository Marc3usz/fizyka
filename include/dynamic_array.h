...
#define array_init(arr, cap, arena)                                                          \
    ...
    {                                                                                        \
        (arr)->capacity = (cap);                                                             \
        (arr)->length = 0;                                                                   \
        (arr)->data = (cap) > 0 ? arena_alloc((arena), (cap) * sizeof(*(arr)->data)) : NULL; \
    } while (0)
...
// Remove element at index by shifting remaining items down
        ...
        {                                                                      \
            ...
            {                                                                  \
                memmove(&(arr)->data[(index)], &(arr)->data[(index) + 1],      \
                        ((arr)->length - (index) - 1) * sizeof(*(arr)->data)); \
            }                                                                  \
            (arr)->length--;                                                   \
        }                                                                      \
    } while (0)
